    ...
    async def shoot(self):
        ...
        for circle in self.wave.circles:
            if sqrt((circle.x - mouse_x)**2 + (circle.y - mouse_y)**2) < circle.r:
                # Initiate burst animation
                initial_color = pygame.Color(255, 0, 0)  # Start with bright red
                for i in range(10):  # Smooth transition effect
                    fading_color = (initial_color.r, max(0, initial_color.g - i*25), max(0, initial_color.b - i*25))
                    pygame.draw.circle(screen, fading_color, (circle.x, circle.y), circle.r + i * 2)
                    pygame.display.flip()
                    #pygame.time.wait(20)  # Delay to make the effect visible
